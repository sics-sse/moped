...
for ( $i = 0 ; $i <= $num_fields; $i++ ) {
    ...
    $content_cells[$i][$ci] = '<select style="font-size: 8pt;" name="field_attribute[]" id="field_' . $i . '_' . ($ci - $ci_offset) . '">' . "\n";
    ...
    if (PMA_MYSQL_INT_VERSION >= 40102) {
        ...
        $content_cells[$i][$ci] .= ' /><label for="field_' . $i . '_' . ($ci - $ci_offset) . 'a" style="font-size: 8pt;">CURRENT_TIMESTAMP</label></div>';
    }
    $ci++;
