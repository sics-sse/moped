...
function PMA_analyseShowGrant($rs_usr, &$is_create_db_priv, &$db_to_create, &$is_reload_priv, &$dbs_where_create_table_allowed) {
    ...
    while ($row = PMA_DBI_fetch_row($rs_usr)) {
        ...
        $show_grants_dbname = preg_replace('/^`(.*)`/', '\\1',  $show_grants_dbname); // CHIP
        ...
        if (($show_grants_str == 'ALL') || ($show_grants_str == 'ALL PRIVILEGES') || ($show_grants_str == 'CREATE') || strpos($show_grants_str, 'CREATE,') !== false) {
            if ($show_grants_dbname == '*') {
				...
				$tempString1 = '/' . $re1 .'(%|_)' . '/';
				$tempStringMod0 = '/' . $re0 . '%' . '/';
				$tempStringMod1 = '/' . $re0 . '_' . '/';
                if((preg_match($tempString0, $show_grants_dbname) && !preg_match('~\\\\%|\\\\_~', $show_grants_dbname))
                 // does this db exist?
                 || (!PMA_DBI_try_query('USE ' .  preg_replace($tempString1, '\\1\\3', $dbname_to_test),  null, PMA_DBI_QUERY_STORE)
                   && substr(PMA_DBI_getError(), 1, 4) != 1044)
                ) {
                    $db_to_create = preg_replace($tempStringMod0, '\\1...', preg_replace($tempStringMod1, '\\1?', $show_grants_dbname));
                    $db_to_create = preg_replace($tempString1, '\\1\\3', $db_to_create);
                    $is_create_db_priv     = true;
