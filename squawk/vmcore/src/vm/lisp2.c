...
#define  printMarkedAddresses() {  \
    ...
    \*---------------------------------------------------------------------------*/
         ...
         */
#define  getBitmapWordIndex(n_218) (  \
             (n_218) >> HDR_LOG2_BITS_PER_WORD  \
        )
         ...
         */
#define  getBitmapBit(n_219) (  \
             (int)((n_219) & BIT_INDEX_MASK)  \
        )
         ...
         */
#define  getBitmapMask(n_220) (  \
             (UWord)1 << getBitmapBit((n_220))  \
        )
         ...
         */
#define  getAddressOfBitmapWordForNthBit(n_221) (  \
             Address_add(bitmapBase, getBitmapWordIndex((n_221)) << HDR_LOG2_BYTES_PER_WORD)  \
        )
         ...
         */
INLINE boolean isNthBitInRange(UWord n_222) {
/*fprintf(stderr, format("isNthBitInRange:\n  base\t= %A\n  bmap\t= %A\n  n\t= %A\n  bmw\t= %A\n"), bitmapBase, bitmap, (n_222), getAddressOfBitmapWordForNthBit((n_222)));*/
            return hieq(getAddressOfBitmapWordForNthBit((n_222)), bitmap) && lo(getAddressOfBitmapWordForNthBit((n_222)), bitmapEnd);
        }
         ...
         */
#define  setNthBit(n_223) { UWord  n_224 = n_223;  \
            UWord index = getBitmapWordIndex((n_224)); \
            UWord mask = getBitmapMask((n_224)); \
            assume(isNthBitInRange((n_224))); \
         ...
         * @param n       the index of the bit to be set
         */
#define  clearNthBit(n_225) { UWord  n_226 = n_225;  \
            UWord index = getBitmapWordIndex((n_226)); \
            UWord mask = ~getBitmapMask((n_226)); \
            assume(isNthBitInRange((n_226))); \
            setUWord(bitmapBase, index, getUWord(bitmapBase, index) & mask); \
        }
         ...
         */
INLINE boolean testNthBit(UWord n_227) {
/*fprintf(stderr, format("testNthBit - n = %A bitmap = %A bitmapWordAddress = %A bitmapWord = %A\n"), (n_227), bitmap, Address_add(bitmapBase, getBitmapWordIndex((n_227))), getUWord(bitmapBase, getBitmapWordIndex((n_227))));*/
            return (getUWord(bitmapBase, getBitmapWordIndex((n_227))) & getBitmapMask((n_227))) != 0;
        }
         ...
         */
INLINE boolean testAndSetNthBit(UWord n_228) {
            UWord oldval;
            assume(isNthBitInRange((n_228)));
            oldval = getUWord(bitmapBase, getBitmapWordIndex((n_228)));
            if ((oldval & getBitmapMask((n_228))) != 0) {
                return true;
            } else {
                setUWord(bitmapBase, getBitmapWordIndex((n_228)), oldval | getBitmapMask((n_228)));
                return false;
            }
        }
         ...
         */
#define  asBitIndex(ea_229) (  \
             (((UWord)(ea_229)) >> HDR_LOG2_BYTES_PER_WORD)  \
        )
         ...
         */
#define  getAddressOfBitmapWordFor(ea_230) (  \
             getAddressOfBitmapWordForNthBit(asBitIndex((ea_230)))  \
        )
         ...
         */
#define  getAddressForBitmapWord(bitmapWordPtr_231) (  \
             (Address)(((UWord)Address_diff((bitmapWordPtr_231), bitmapBase) << HDR_LOG2_BITS_PER_BYTE) << HDR_LOG2_BYTES_PER_WORD)  \
        )
         ...
         */
#define  testBitFor(ea_232) (  \
             testNthBit(asBitIndex((ea_232)))  \
        )
...
#define  setBitFor(ea_233) { Address  ea_234 = ea_233;  \
 \
 \
 \
/*fprintf(stderr, format("setBitFor - ea = %A *ea = %A\n"), (ea_234), *((UWord*)(ea_234)));*/ \
/*if ((UWord)(ea_234) == BAD_ADDRESS) fprintf(stderr, format("setBitFor - ea = %A *ea = %A\n"), (ea_234), *((UWord*)(ea_234)));*/ \
/*if (*((UWord*)(ea_234)) == 109609828) fatalVMError("bad set bit");*/ \
            setNthBit(asBitIndex((ea_234))); \
            assume(testBitFor((ea_234))); \
...
/*printMarkedAddresses();*/ \
         ...
         * @param ea      the effective address for which the corresponding bit is to be set
         */
#define  clearBitFor(ea_235) { Address  ea_236 = ea_235;  \
            clearNthBit(asBitIndex((ea_236))); \
        }
         ...
         */
INLINE boolean testAndSetBitFor(Address ea_237) {
/*fprintf(stderr, format("testAndSetBitFor - ea = %A\n"), (ea_237));*/
            return testAndSetNthBit(asBitIndex((ea_237)));
        }
