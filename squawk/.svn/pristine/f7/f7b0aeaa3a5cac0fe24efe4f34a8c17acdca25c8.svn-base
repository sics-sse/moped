...
public final class String {
     ...
     * Note caller must check parameters.
     */
    private static void stringcopy(Object src, int src_position, Object dst, int dst_position, int totalLength) {
        ...
        if (false) Assert.that(GC.getKlass(src).isSquawkArray());
        if (false) Assert.that(GC.getKlass(dst).isSquawkArray());
        if (false) Assert.that((totalLength  >= 0) &&
                    (src_position >= 0) &&
                    (dst_position >= 0));
        if (false) Assert.that((totalLength == 0 || ((totalLength + src_position) >= 0 && (totalLength + dst_position) >= 0)) &&
     ...
     * a subarray of the src argument. The <code>offset</code>
     * argument is the index of the first character of the subarray and
     * the <code>count</code> argument specifies the length of the
     * subarray. The contents of the subarray are copied; subsequent
     * modification of the src does not affect the newly
     * created string.
     *
     * @param      src      string (8 or 16 bit) or array of char, that is the source of characters.
     * @param      offset   the initial offset.
     * @param      count    the length.
     * @exception  IndexOutOfBoundsException  if the <code>offset</code>
     *               and <code>count</code> arguments index characters outside
     *               the bounds of the <code>src</code> array.
     * @exception NullPointerException if <code>src</code> is
     *               <code>null</code>.
     */
    ...
    private static String init(char[] src, int offset, int count) {
        return init(src, offset, count, isEightBitEnc(src, offset, count));
    }
    ...
    static String _init_(String self, byte bytes[], int off, int len, String enc) throws UnsupportedEncodingException, ReplacementConstructorPragma  {
        return initEncoded(bytes, off, len, enc);
    }
    ...
    private static String initEncoded(byte bytes[], int off, int len, String enc) throws UnsupportedEncodingException {
        if (Helper.isISO8859_1(enc)) {
            return init(bytes, off, len, true); // directly convert bytes to string
        } else {
            if (Helper.ISO8859_1_ONLY_SUPPORTED) {
                throw new UnsupportedEncodingException(enc);
            } else {
                char[] value = Helper.byteToCharArray(bytes, off, len, enc); //ensures enc is supported
                return init(value, 0, value.length);
            }
        }
    }
    ...
    static String _init_(String self, byte bytes[], String enc) throws UnsupportedEncodingException, ReplacementConstructorPragma {
		return initEncoded(bytes, 0, bytes.length, enc);
    }
    ...
    static String _init_(String self, byte bytes[], int off, int len) throws ReplacementConstructorPragma {
        if (Helper.ISO8859_1_ONLY_SUPPORTED) {
            // optimize the only case, which is trivial:
            return init(bytes, off, len, true);
        } else {
    ...
    static String _init_(String self, byte bytes[]) throws ReplacementConstructorPragma {
        if (Helper.ISO8859_1_ONLY_SUPPORTED) {
            // optimize the only case, which is trivial:
            return init(bytes, 0, bytes.length, true);
        } else {
    ...
    public void getChars(int srcBegin, int srcEnd, char dst[], int dstBegin) {
        ...
        if (dstBegin < 0 || dstEnd < 0 || dstEnd > dst.length) { // check for overflow on dstEnd
            throw new ArrayIndexOutOfBoundsException();
        }
        stringcopy(this, srcBegin, dst, dstBegin, count);
    }
...
//        }
    ...
    public boolean equalsIgnoreCase(String anotherString) {
        int len = length();
        return (anotherString != null) && (anotherString.length() == len) &&
    ...
    public String concat(String str) {
        ...
        // @todo could optimize (especially 8bit case), to not allocate temp buffer, but
        // construct and write into resulting string. See init(String).
        int thisLen = this.length();
        char buf[] = new char[thisLen + otherLen];
        getChars(0, thisLen, buf, 0);
