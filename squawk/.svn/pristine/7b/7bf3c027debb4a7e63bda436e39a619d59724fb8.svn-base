...
{ 
    ...
    {
	checkDestroyed();
	synchronized (recordStore) { // don't want rs to change here
...
rse.printStackTrace();
	...
	for (i = 0, j = 1; i < records.length - 1; i++, j++) {
        byte[] data1 = recordStore.getRecord(records[i]);
        byte[] data2 = recordStore.getRecord(records[j]);
	    if (comparator.compare(data1, data2) == RecordComparator.FOLLOWS) {
