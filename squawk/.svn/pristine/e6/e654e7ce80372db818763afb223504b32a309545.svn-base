...
public class GC implements GlobalStaticFields {
    ...
    static void registerReadOnlyObjectMemory(ObjectMemory om) {
        if (false) Assert.that(!GC.inRam(om.getStart()));
        if (false) Assert.that(lookupReadOnlyObjectMemoryByRoot(om.getRoot()) == null);
    ...
    static Address allocateNvmBuffer(int size) {
        if (false) Assert.that(size == roundUpToWord(size));
    ...
    static Offset getOffsetInNvm(Address object) {
        if (false) Assert.that(inNvm(object));
    ...
    static void initialize() throws HostedPragma {
        ...
        Assert.always(ramStart.eq(ramStart.roundUpToWord()), "RAM limit is not word aligned", "GC.java", 552);
        Assert.always(ramEnd.eq(ramEnd.roundDownToWord()), "RAM limit is not word aligned", "GC.java", 553);
    ...
    static void initialize(Suite bootstrapSuite) {
        if (false) Assert.that(!VM.isHosted());

        // Get the pointers rounded correctly.
        Assert.always(ramStart.eq(ramStart.roundUpToWord()), "GC.java", 571); // "RAM limit is not word aligned"
        Assert.always(ramEnd.eq(ramEnd.roundDownToWord()), "GC.java", 572);   // "RAM limit is not word aligned"
        ...
        GarbageCollector newcollector = new com.sun.squawk.CheneyCollector(ramStart, ramEnd);
...
//    }
    ...
    static Offset getOffsetInRam(Address object) {
        if (false) Assert.that(inRam(object));
    ...
    public static boolean inRam(Address start, Address end) {
        ...
        Assert.always(userRangeSize.ge(Offset.zero()), "GC.java", 721);
    ...
    private static Object allocatePrim(int size, Object klass, int arrayLength) {
        if (false) Assert.that(allocationEnabled);
        if (false) Assert.that(size == roundUpToWord(size));
        if (false) Assert.that(size >= 0);
        /*
...
//                VM.print(VM.branchCount());
    ...
    private static Object allocate(int size, Object klass, int arrayLength) {
        ...
        if (false) Assert.that(VM.isHosted() || !inRam(klass) || Address.fromObject(klass).lo(Address.fromObject(oop)));
        return oop;
    }
...
////    }
    ...
    public static Klass getKlass(Object object)  throws ForceInlinedPragma {
        if (false) Assert.that(object != null);
...
//        Assert.always(!classOrAssociation.isZero());
        ...
        if (false) Assert.that(klass != null);
    ...
    public static int getArrayLength(Object array) throws ForceInlinedPragma {
        if (false) Assert.that(Klass.isSquawkArray(getKlass(array)));
...
//        
    ...
    static Object newMethod(Object definingClass, MethodBody body) {
        ...
        if (false) Assert.that(GC.roundUpToWord(hsize) == hsize);
        ...
        if (false) Assert.that((hsize & HDR.headerTagMask) == 0);
...
//        body.verifyMethod(oop);
    ...
    public static void arraycopy(Object src, int srcPos, Object dst, int dstPos, int lth) {
        if (false) Assert.that(GC.getKlass(src).isArray());
        if (false) Assert.that(GC.getKlass(dst).isArray());
        int itemLength = Klass.getComponentType(GC.getKlass(src)).getDataSize(); // static inlines better
        if (false) Assert.that(GC.getKlass(dst).getComponentType().getDataSize() == itemLength);
    ...
    public static void stringcopy(Object src, int srcPos, Object dst, int dstPos, int lth) {
        ...
        if (srcsize == getStringOperandSize(dst)) {
            ...
            if (false) Assert.that(getStringOperandSize(dst) == 2);
            for (int i = 0 ; i < lth ; i++) {
                int ch = NativeUnsafe.getByte(src, srcPos++) & 0xFF;
                NativeUnsafe.setChar(dst, dstPos++, ch);
            }
       } else {
            if (false) Assert.that(srcsize == 2 && getStringOperandSize(dst) == 1);
            for (int i = 0 ; i < lth ; i++) {
    ...
    private static ObjectAssociation getObjectAssociation(Object object) throws ForceInlinedPragma {
        if (false) Assert.that(GC.inRam(object));
    ...
    private static ObjectAssociation createObjectAssociation(Object object, Klass klass) {
        ...
        if (false) Assert.that(VM.isHosted() || Address.fromObject(object).lo(Address.fromObject(assn)));
...
//VM.println(VM.branchCount());
    ...
    static String findInRomString(String string) {
        ...
        while (parent == null) {
            if (false) Assert.that(suite != null);
   ...
   /**
    ...
    static int getObjectBytes(Object object) {
        Klass klass = GC.getKlass(object);
        if (false) Assert.that(Klass.getSystemID(klass) != CID.BYTECODE_ARRAY); //  Doesn't calculate headers sizes for methods correctly
...
//                if (false) Assert.that(gaklass != null);
