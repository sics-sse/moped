...
//    private Klass ObjectMemoryKlass;
    ...
    void initialize(Address ignored, Address memoryStart, Address memoryEnd) {
        ...
        Assert.always(heapSize > 0, "CheneyCollector.java", 175);
         ...
         */
        if (false) Assert.that(fromSpaceEndPointer.diff(fromSpaceStartPointer).eq(toSpaceEndPointer.diff(toSpaceStartPointer)), "semi-spaces are different sizes");
    ...
    private void setForwardPointer(Address object, Address forwardPointer) {
        if (copyingObjectGraph) {
            ...
            if (false) Assert.that(fromSpaceEndPointer.diff(forwardingRepairMapTop).ge(Offset.fromPrimitive(HDR.BYTES_PER_WORD * 2)), "forwarding repair map is too small");
    ...
    private static Address getForwardedObject(Address object) {
        ...
        if (false) Assert.that(classWord.and(UWord.fromPrimitive(HDR.forwardPointerBit)).ne(Address.zero()), "object is not forwarded");
    ...
    private Address copyObject(final Address object) {
         ...
         */
        if (false) Assert.that(object.roundUpToWord().eq(object), "cannot copy unaligned object");
         ...
         */
        if (false) Assert.that(object.toObject() != this, "cannot copy the CheneyCollector object");
        ...
        if (isForwarded(object)) {
            ...
            if (false) Assert.that(forwardPointer.roundUpToWord().eq(forwardPointer), "unaligned forward pointer");
        ...
        if (false) Assert.that(GC.blockToOop(block).eq(object), "mis-sized header for copied object");
...
//        }
        ...
        if (false) Assert.that(copiedObject.roundUpToWord().eq(copiedObject), "unaligned copied object");
...
//    }
    ...
    private Address updateReference(Address base, int offset) {
        if (false) Assert.that(base.roundUpToWord().eq(base), "unaligned base address");
    ...
    private void updateActivation(Address fp, boolean isInnerMostActivation) {
        ...
        if (!previousIP.isZero()) {
             ...
             * Adjust the MP
             */
            if (false) Assert.that(!previousFP.isZero(), "activation frame has null previousFP");
            Address oldPreviousMP = NativeUnsafe.getAddress(previousFP, FP.method);
            if (false) Assert.that(!isInToSpace(oldPreviousMP), "a method was copied before activation frame of the method");
            ...
            if (false) Assert.that(previousFP.isZero(), "previousFP should be null when previousIP is null");
        }
    ...
    private void updateStackChunk(Address chunk) {
         ...
         */
        Assert.always(NativeUnsafe.getAddress(chunk, SC.next).isZero(), "CheneyCollector.java", 855);
            ...
            switch (Klass.getSystemID(klass)) {
                ...
                case CID.GLOBAL_ARRAY: {
                    ...
                    if (gaklass == null) { // This can occur if a GC occurs when a class state is being allocated
                        ...
                        for (int i = 0 ; i < end ; i++) {
                            if (false) Assert.that(i < GC.getArrayLength(object), "class state index out of bounds");
    ...
    private Address copyNonRootObjects(Address toSpaceUpdatePointer) {
        ...
        while (toSpaceUpdatePointer.lo(toSpaceAllocationPointer)) {
            ...
            if (false) Assert.that(GC.oopToBlock(klass, object).eq(toSpaceUpdatePointer), "bad size for copied object");
            updateOops(object);
...
//    }
    ...
    private void processWeakReferenceQueue() {
        ...
        while (ref != null) {
            ...
            if (false) Assert.that(isInFromSpace(refAddress));
            Ref next = ref.next;
