...
public final class Lisp2GenerationalCollector extends GarbageCollector {
    ...
    Lisp2GenerationalCollector(Address ramStart, Address ramEnd) {
        ...
        Assert.always(ramSize > 0, "Lisp2GenerationalCollector.java", 417);
...
////      copyTimer = new Timer(this);
    ...
    private static int calculateBitmapSize(int memorySize) {
        Assert.always(GC.roundDownToWord(memorySize) == memorySize, "Lisp2GenerationalCollector.java", 479);
    ...
    private static int calculateMaxHeapSize(int memorySize) {
        ...
        Assert.always(memorySize >= heapSize + calculateBitmapSize(heapSize), "Lisp2GenerationalCollector.java", 500);
        return heapSize;
    }
    ...
    void initialize(Address permanentMemoryStart, Address memoryStart, Address memoryEnd) {
        ...
        Assert.always(memoryEnd.roundUpToWord().eq(memoryEnd), "Lisp2GenerationalCollector.java", 509);
        Assert.always(memoryStart.roundUpToWord().eq(memoryStart), "Lisp2GenerationalCollector.java", 510);
        Assert.always(permanentMemoryStart.roundUpToWord().eq(permanentMemoryStart), "Lisp2GenerationalCollector.java", 511);
    ...
    private void initializeHeap() {
        ...
        Assert.always(memorySize > 0, "insufficient memory for collector", "Lisp2GenerationalCollector.java", 547);
        ...
        Assert.always(heapSize >= alignment, "insufficient memory for collector", "Lisp2GenerationalCollector.java", 560);

        // Align the heap size
        heapSize = GC.roundUp(heapSize, alignment);
        heapEnd = heapStart.add(heapSize);
        Assert.always(heapSize > 0, "insufficient memory for collector", "Lisp2GenerationalCollector.java", 565);
...
//            VM.print("heapOffsetBits: "); VM.print(bitsRequiredFor(heapSize / HDR.BYTES_PER_WORD)); VM.println();
        ...
        Assert.always(sliceTable.add(sliceTableSize).loeq(alignedMemoryEnd), "slice table overflows memory boundary", "Lisp2GenerationalCollector.java", 634);
        Assert.always(sliceOffsetBits < 32, "slice size overflows 32 bit address space", "Lisp2GenerationalCollector.java", 635);
        Assert.always(bitmap.add(bitmapSize).loeq(sliceTable), "bitmap collides with slice table", "Lisp2GenerationalCollector.java", 636);
        Assert.always(heapEnd.loeq(bitmap.sub(minimumMarkingStackSize)), "heap collides with marking stack", "Lisp2GenerationalCollector.java", 637);
        Assert.always((heapSize % alignment) == 0, "heap size is non-aligned", "Lisp2GenerationalCollector.java", 638);
        Assert.always((bitmapSize % HDR.BYTES_PER_WORD) == 0, "bitmap size is non-aligned", "Lisp2GenerationalCollector.java", 639);
        Assert.always((sliceTableSize % HDR.BYTES_PER_WORD) == 0, "slice table size is non-aligned", "Lisp2GenerationalCollector.java", 640);
        Assert.always(Lisp2Bitmap.getAddressOfBitmapWordFor(permanentMemoryStart).eq(Lisp2Bitmap.getStart()), "incorrect bitmap base", "Lisp2GenerationalCollector.java", 641);
        Assert.always(Lisp2Bitmap.getAddressOfBitmapWordFor(heapEnd).lo(Lisp2Bitmap.getEnd()), "incorrect bitmap base", "Lisp2GenerationalCollector.java", 642);
    }
    ...
    public int getYoungGenerationSize() {
        ...
        if (available < size) {
            if (false) Assert.that((available % HDR.BYTES_PER_WORD) == 0);
            return available;
        } else {
            if (false) Assert.that((size % HDR.BYTES_PER_WORD) == 0);
            return size;
        }
    }
...
////    }
    ...
    private void registerStackChunk(Object chunk) {
        Assert.always(NativeUnsafe.getObject(chunk, SC.owner) != null, "Lisp2GenerationalCollector.java", 1078);
        Assert.always(NativeUnsafe.getAddress(chunk, SC.next).isZero(), "Lisp2GenerationalCollector.java", 1079);
        GC.checkSC(chunk);

        if (false) Assert.that(!findStackChunk(stackChunks, chunk), "stack chunk already exists in list of stack chunks");
...
//VM.println(VM.branchCount());
    ...
    void deregisterStackChunk(Object chunk) {
        verifyStackChunks();
        if (false) Assert.that(chunk != null);
        if (false) Assert.that(stackChunks != null);
        GC.checkSC(chunk);
        if (chunk == stackChunks) {
            ...
            while (NativeUnsafe.getObject(prev, SC.next) != chunk) {
                ...
                if (false) Assert.that(prev != null);
            }
...

    {
        ...
        Address object;
        if (false) Assert.that(GC.inRam(base, base));
...
//                Assert.shouldNotReachHere("illegal oop traversal phase");
    ...
    private void traverseOopsInObject(Address object, Address klass, int visitor) {
        ...
        if (visitor == MARK_VISITOR) {
            ...
            if (false) Assert.that(visitor == UPDATE_VISITOR);
            if (isForwarded(object)) {
        ...
        switch (Klass.getSystemID(VM.asKlass(klass))) {
            ...
            case CID.GLOBAL_ARRAY: {
                ...
                if (gaklass == null) { // This can occur if a GC occurs when a class state is being allocated
                    if (false) Assert.that(NativeUnsafe.getObject(object, CS.next) == null); // The 'next' pointer should not yet be set either
                    if (GC.GC_TRACING_SUPPORTED && tracing()) {
                    ...
                    if (false) Assert.that(GC.inRam(object, object));
...
//VM.println();
                    if (false) Assert.that(NativeUnsafe.getUWord(object, (int)FieldOffsets.com_sun_squawk_Klass$oopMapWord).eq(UWord.zero()), "oopMapWord not zero'ed by last collection");
...
////            || !isForwarded(NativeUnsafe.getAddress(object, (int)FieldOffsets.com_sun_squawk_Klass$oopMap)));
        ...
        if (instanceSize > HDR.BITS_PER_WORD) {
            if (visitor == UPDATE_VISITOR && !oopMapWord.isZero()) {
                if (false) Assert.that(!copyingObjectGraph, "oop map backup pointer should not be made during an object graph copy ");
                /*
                ...
                if (false) Assert.that(oopMapWord.isZero(), "oopMapWord not zero'ed by last collection");
...
//                visitOop(visitor, chunk, SC.next);
                ...
                if (false) Assert.that(UPDATE_VISITOR == visitor);
    ...
    private void traverseOopsInActivation(Address fp, boolean isInnerMostActivation, int visitor) {
        ...
        if (false) Assert.that(GC.inRam(fp, fp));
        ...
        if (!returnIP.isZero()) {
            ...
            if (false) Assert.that(!returnFP.isZero(), "activation frame has null returnFP");
...
////          Assert.that(!inHeap(returnMP)); // returnMP will never move, and will always be live.
            ...
            if (false) Assert.that(returnFP.isZero(), "returnFP should be null when returnIP is null");
        }
    ...
    private void traverseWriteBarrierOops(Address start, Address end, int visitor) throws AllowInlinedPragma {
        if (false) Assert.that(GC.inRam(start, end));
        ...
        while (!(objectAddress = Lisp2Bitmap.Iterator.getNext()).isZero()) {
            ...
            if (false) Assert.that(GC.inRam(objectAddress, objectAddress));
   ...
   /**
    ...
    private void mark() {
        ...
        if (false) Assert.that(!copyingObjectGraph);
        timer.reset();
...
//VM.print(((VMThread)NativeUnsafe.getObject(sc, SC.owner)).getName());
    ...
    private void pruneStackChunks() throws NotInlinedPragma {
        Assert.always(isFullCollection(), "Lisp2GenerationalCollector.java", 2153);
        ...
        while (!sc.isZero()) {
            GC.checkSC(sc);
            next = NativeUnsafe.getAddress(sc, SC.next);
            if (false) Assert.that(NativeUnsafe.getObject(sc, SC.owner) != null);
...
////    }
    ...
    private void markObject(Address object) {
        ...
        */
        if (false) Assert.that(!isForwarded(object));
...
////                    }
    ...
    private void killObject(Address object, int size) {
        if (false) Assert.that(size >= HDR.basicHeaderSize);
        if (false) Assert.that(size == GC.roundUpToWord(size));
        ...
        while (!(object = Lisp2Bitmap.Iterator.getNext()).isZero()) {
            ...
            if (!delta.isZero()) {
                ...
                if (slopAllowed.ge(delta)) {
					...
					//if (false) Assert.that(klass == GC.getKlass(object)); // make sure we didn't stomp on this object
                    if (GC.GC_TRACING_SUPPORTED && tracing()) {
                    ...
                    if (false) Assert.that(getForwardedObject(object).eq(objectDestination));
...
////    }
    ...
    private Offset getOffsetInHeap(Address object) throws AllowInlinedPragma {
        if (false) Assert.that(inHeap(object));
    ...
    private UWord encodeForwardingPointer(Address object, Address destination) {
        ...
        if (false) Assert.that(offsetInSlice.ge(Offset.zero()) && offsetInSlice.lt(Offset.fromPrimitive(1 << sliceOffsetBits)));
    ...
    private UWord encodeClassOrAssociationPointer(Address classOrAssociation) {
        ...
        if (inHeap(classOrAssociation)) {
            ...
            if (false) Assert.that(GC.inNvm(classOrAssociation));
        ...
        if (false) Assert.that(classOrAssociationOffset.ge(Offset.zero()) && classOrAssociationOffset.lt(Offset.fromPrimitive(1 << classOffsetBits)),
    ...
    private Address decodeClassOrAssociationPointer(UWord encodedClassOrAssociation) {
        ...
        switch (tag) {
            ...
            default:
                if (Assert.SHOULD_NOT_REACH_HERE_ALWAYS_ENABLED) Assert.shouldNotReachHere("Lisp2GenerationalCollector.java", 3015);
                return Address.zero();
        }
    }
    ...
    private static boolean isClassPointer(Address object, Address classOrAssociation) throws AllowInlinedPragma {
        if (false) Assert.that(classOrAssociation.ne(object));
...
////      return !GC.inRam(classOrAssociation);
    ...
    private Address getKlass(Address object) {
        ...
        if (false) Assert.that(!classOrAssociation.isZero());
        if (false) Assert.that(!classOrAssociation.eq(Address.fromPrimitive(0xDEADBEEF)));
...
//            if (false) Assert.that(klass.ne(classOrAssociation));
//        } else {
//            klass = classOrAssociation;
//        }

        if (false) Assert.that(!klass.isZero());
        if (false) Assert.that(!klass.eq(Address.fromPrimitive(0xDEADBEEF)));
        return klass;
    }
    ...
    private static void printKlassName(Object klass) {
        ...
        if (isForwarded(klassAddress)) {
            ...
            if (false) Assert.that(!GC.inRam(klassAddress) || Address.fromObject(name).lo(klassAddress));
            VM.print(name);
        }
    }
    ...
    private int getSliceIndexForObject(Address object) {
        ...
        if (false) Assert.that(index >= 0 && index < sliceCount);
        return index;
    }
...
/////*end[DEBUG_CODE_ENABLED]*/
    ...
    private Address compactObjects(Address firstDeadBlock, Address firstMovingBlock) throws NotInlinedPragma {
        ...
        if (false) Assert.that(!firstDeadBlock.isZero());
        ...
        while (!(object = Lisp2Bitmap.Iterator.getNext()).isZero()) {
            ...
            if (isClassPointer(object, classOrAssociation)) {
                ...
                if (false) Assert.that(isForwarded(classOrAssociation));
...
//            }
            ...
            if (false) Assert.that(!isForwarded(objectDestination));
        }
...
//        if (false) Assert.that(stackInUse);
            if (false) Assert.that(!object.isZero());
