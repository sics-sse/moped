 ...
 * (DateParser will not handle dates prior to 1/1/1970, and to
 * be able to provide methods that will set timezone and DST
 * information, it is called DateParser.
 *
 * Update: Only used by http class, which deals with GMT, so assume GMT.
 */
...
//    }
     ...
     */
    private long computeJulianDay(int inp_year, 
                                        int inp_month, 
                                        int inp_day) {
        int y;
        ...
        y = inp_year - 1;
        long julianDay = 365L*y + CalendarImpl.floorDivide(y, 4) + (JAN_1_1_JULIAN_DAY - 3);
        
        isLeap = isLeap && ((inp_year%100 != 0) || (inp_year%400 == 0));
        // Add 2 because Gregorian calendar starts 2 days after Julian calendar
        julianDay += CalendarImpl.floorDivide(y, 400) - CalendarImpl.floorDivide(y, 100) + 2;
