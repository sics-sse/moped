...
public class MethodHeader {
    ...
    private static int minfoValue(Object oop, int offset) throws NotInlinedPragma {
        ...
        int val = -1;
        if (false) Assert.that((NativeUnsafe.getUByte(oop, p+1) & FMT_LARGE) != 0);
        while(offset-- > 0) {
    ...
    private static int minfoValue4(Object oop) throws NotInlinedPragma  {
        int p = HDR.methodInfoStart - 1;
        int val;
        if (false) Assert.that(((NativeUnsafe.getByte(oop, p+1)) & FMT_LARGE) != 0);
        if (NativeUnsafe.getByte(oop, p--) < 0) {
            p--;
        }
        if (NativeUnsafe.getByte(oop, p--) < 0) {
        ...
        if (decodeTypeTableSize(oop) > 0) {
            ...
            while (dec.getOffset() < end) {
                ...
                switch (cid) {
                    ...
                    default: if (Assert.SHOULD_NOT_REACH_HERE_ALWAYS_ENABLED) Assert.shouldNotReachHere("MethodHeader.java", 467);
                }
            }
        }
        return types;
