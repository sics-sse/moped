 ...
 * Copyright 2011-2012 Oracle Corporation. All Rights Reserved.
...
//        LineReader in = new LineReader(new InputStreamReader(inStream, "8859_1"));
        LineReader in = new LineReader(new InputStreamReader(inStream));
        while (true) {
            // Get next line
     ...
     * @param header A header to be put on the front of the output, or null if no header required
     * @throws IOException
     */
    public synchronized void store(OutputStream out, String header) throws IOException {
    	OutputStreamWriter awriter;
        awriter = new OutputStreamWriter(out);
        if (header != null) {
            writeln(awriter, "#" + header);
        }
        // This is may be the only default use of Date classes in mbed, so don't use it.
     ...
     * @see     com.sun.spot.core.util.Properties#defaults
     */
