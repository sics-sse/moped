 ...
 *
 * <p>For Sun SPOTs the following additional time zones are supported:
 * <pre>
 *     HST, AST, PST, MST, CST & EST and "GMT+hh:mm"
 * </pre>
 * plus we allow other timezones to be user defined via system properties 
 * passed in to Squawk on the command line.
 *
 * <p>The default time zone is "PST". It can be changed to another supported
 * time zone by setting the system property
 *     <code>com.sun.cldc.util.j2me.TimeZoneImpl.timezone</code>
 ...
 * </pre>
 *
 * <p>To add additional time zones use the system property
 *     <code>com.sun.cldc.util.j2me.TimeZoneImpl.additionaltimezones</code>,
 *  where its value is a list of the time zones to add in the format:
 * <pre>
 *     tzname1={+-}hh{:mm}{,<i>daylight_savings_spec</i>};tzname2=...;
 * </pre>
 * where 
 * <pre>
 *     <i>daylight_savings_spec</i> =
 *           <i>start_month</i>,<i>start_day</i>,<i>start_day_of_week</i>,<i>start_time</i>,
 *           <i>end_month</i>,<i>end_day</i>,<i>end_day_of_week</i>,<i>end_time</i>,hh{:mm}
 * </pre>
 * <p>To specify the last Sunday of the month set the <i>day_of_week</i> to Calendar.SUNDAY 
 * and the <i>day</i> to -1, i.e. <i>day</i> is interpreted then to be which Sunday (1 = first,
 * 2 = second, -2 = second last, etc.). 
 *
 * <p>It is also possible to specify a date such as the first Sunday before/after a 
 * given day of the month by setting the <i>day_of_week</i> to be negative. If the 
 * <i>day</i> value is negative then the <i>day_of_week</i>  before that day is specified.
 *
 * <p>For example an Australian might define:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Refer to the time zone defining code at the end of this file for examples of 
 * the values to use, e.g.:
 *
 * <pre>
...
public class TimeZoneImpl extends GMTImpl {
    ...
    /**
     * Constructs a TimeZone with the given base time zone offset from GMT
     * and time zone ID that uses the specified rule for daylight savings time.
     *
     * @param rawOffset  The given base time zone offset to GMT.
     * @param ID         The time zone ID which is obtained from
     *                   TimeZone.getAvailableIDs.
     * @param startMonth The month in which daylight savings time starts [0:11].
     * @param startDay   If startDayOfWeek is zero then the day on which daylight 
     *                   savings time starts [0:28, 30 or 31]. 
     *                   If startDayOfWeek is greater than zero then the week in
     *                   which daylight savings time starts: e.g. 1 = first Sunday
     *                   in month, 2 = second, -1 = last.
     *                   If startDayOfWeek is less than zero then the week in
     *                   which daylight savings time starts: e.g. 15 = Sunday
     *                   after the 15th, -12 = Sunday before the 12th.
     * @param startDayOfWeek The day of the week on which daylight savings time
     *                   starts [-7:7]. 
     * @param startTime  The hour and minute that daylight savings time starts [hh:mm]
     * @param endMonth   The month in which daylight savings time ends [0:11].
     * @param endDay     If endDayOfWeek is zero then the day on which daylight 
     *                   savings time ends [0:28, 30 or 31]. 
     *                   If endDayOfWeek is greater than zero then the week in
     *                   which daylight savings time ends: e.g. 1 = first Sunday
     *                   in month, 2 = second, -1 = last.
     *                   If endDayOfWeek is less than zero then the week in
     *                   which daylight savings time ends: e.g. 15 = Sunday
     *                   after the 15th, -12 = Sunday before the 12th.
     * @param endDayOfWeek The day of the week on which daylight savings time
     *                   ends [-7:7]. 
     * @param endTime    The hour and minute that daylight savings time ends [hh:mm]
    ...
    /**
     * Parses one time zone definition:
     *
     *    tzname1={-}hh{:mm}{,<daylight savings spec>};
     *
     * where <daylight savings spec> =
     *       <start month>,{-}<start day>,{-}<start day of week>,<start time>,
     *       <end month>,{-}<end day>,{-}<end day of week>,<end time>,hh{:mm}
     *
     * @param s definition to parse
     * @return a TimeZoneImpl, if no errors during parsing
     */
    private static TimeZoneImpl parseTimeZone(String str) {
        ...
        int dst = str.indexOf(',', i+1);
        int gmtOffset;
        if (dst < 0) {    // no daylight savings spec
            gmtOffset = parseGMToffset(str, i+1, str.length());
            return (gmtOffset == -1) ? null : new TimeZoneImpl(gmtOffset, name);
        } else {
            gmtOffset = parseGMToffset(str, i+1, dst);
            if (gmtOffset == -1) return null;
            int index[] = new int[1];
            index[0] = dst + 1;
            int startMonth = parseInt(str, index);
            if (startMonth == -999) return null;
            int startDay = parseInt(str, index);
            if (startDay == -999) return null;
            int startDayOfWeek = parseInt(str, index);
            if (startDayOfWeek == -999) return null;
            i = str.indexOf(',', index[0] + 1);
            int startTime = parseGMToffset(str, index[0], i);
            if (startTime == -1) return null;
            index[0] = i + 1;
            int endMonth = parseInt(str, index);
            if (endMonth == -999) return null;
            int endDay = parseInt(str, index);
            if (endDay == -999) return null;
            int endDayOfWeek = parseInt(str, index);
            if (endDayOfWeek == -999) return null;
            i = str.indexOf(',', index[0] + 1);
            int endTime = parseGMToffset(str, index[0], i);
            if (endTime == -1) return null;
            int dstSavings = parseGMToffset(str, i + 1, str.length());
            if (dstSavings == -1) return null;
            return new TimeZoneImpl(gmtOffset, name, 
                                    startMonth, startDay, startDayOfWeek, startTime,
                                    endMonth, endDay, endDayOfWeek, endTime, dstSavings);
        }
    }
    ...
    /**
     * Make sure any time zones specified via the system property:
     *
     *    com.sun.cldc.util.j2me.TimeZoneImpl.additionaltimezones
     *
     * are added to the list of defined time zones.
     */
    private static void init() {
        ...
        if (tzDefs != null) {
            ...
            while (fromIndex < tzDefs.length()) {
                ...
                int j = tzDefs.indexOf(';', fromIndex);
                if (j < 0) {
                    tz = parseTimeZone(tzDefs.substring(fromIndex));
                    fromIndex = tzDefs.length();
                } else {
                    tz = parseTimeZone(tzDefs.substring(fromIndex, j));
                    fromIndex = j + 1;
                }
                if (tz != null) {
    ...
    private static TimeZoneImpl parseCustomTimeZone(String id) {
        ...
        char c = id.charAt(GMT_ID_LENGTH);
        if (c != '-' && c != '+') {
            return null;
        }
        int gmtOffset = parseGMToffset(id, GMT_ID_LENGTH, id.length());
        if (gmtOffset == -1) {
            return null;
        }
    ...
    static TimeZoneImpl zones[] = {
        //----------------------------------------------------------
        new TimeZoneImpl(0*ONE_HOUR, "GMT"),
        // GMT  -(-)    0:00    -   GMT
        new TimeZoneImpl(0*ONE_HOUR, "UTC"),

        // support North American generic time zones: HST, AST, PST, MST, CST & EST
        new TimeZoneImpl(-10*ONE_HOUR, "HST"),
        new TimeZoneImpl(-9*ONE_HOUR, "AST",
                Calendar.MARCH, 2, Calendar.SUNDAY, 2*ONE_HOUR,
                Calendar.NOVEMBER, 1, -Calendar.SUNDAY, 2*ONE_HOUR, 1*ONE_HOUR),
    ...
    public boolean equals(Object object) {
        if (!(object instanceof TimeZoneImpl)) return false;
        TimeZoneImpl obj = (TimeZoneImpl)object;
 
        boolean retValue = this.getID().equals(obj.getID()) && 
                           this.rawOffset == obj.rawOffset && 
                           this.useDaylight == obj.useDaylight;
        
        if (this.useDaylight && retValue) {
            retValue = this.startMonth == obj.startMonth &&
                       ...
                       this.dstSavings == obj.dstSavings;
        }
