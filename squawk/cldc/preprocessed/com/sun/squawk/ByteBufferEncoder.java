...
public final class ByteBufferEncoder {
    ...
    protected final void ensure(int needed) {
        if (false) Assert.that(needed > 0);
        int remain = buffer.length - count;
        if (false) Assert.that(remain >= 0);
        if (remain < needed) {
        ...
        if (false) Assert.that(buffer.length - count >= needed);
    }
    ...
    public final void addUnsignedByte(int value) {
        if (false) Assert.that(value >= 0 && value <= 0xFF);
    ...
    public final void addUnsignedShort(int value) {
        if (false) Assert.that(value >= 0 && value <= 0xFFFF);
    ...
    void addUnsignedInt(int value) {
        if (false) Assert.that(value >= 0 && value < 0x0FFFFFFF);
        if (value < 128) {
            /* 0xxxxxxx */
            ensure(1);
            buffer[count++] = (byte)value;
...
//        }
    ...
    public void addUtf8(String s) {
        ...
        if (false) Assert.that(equalsUtf8(s, start));
    }
    ...
    public void add(ByteBufferEncoder buf) {
        ...
        if (false) Assert.that(bufcount > SymbolParser.STATIC_METHODS, "invalid member length");
