...
//    }
    ...
    public ObjectMemory findAddress(Address address) {
        if (address.hieq(start)) {
            if (false) Assert.that(address.lo(getEnd()), "address is out of range");
            return this;
        }
        if (false) Assert.that(parent != null, "address is out of range");
    ...
    public ObjectMemory findCanonicalAddress(Address canonicalAddress) {
        if (canonicalAddress.hieq(canonicalStart)) {
            if (false) Assert.that(canonicalAddress.lo(getCanonicalEnd()), "address is out of range");
            return this;
        }
        if (false) Assert.that(parent != null, "address is out of range");
    ...
    public Address toCanonical(Address address) {
        ...
        if (false) Assert.that(delta.toPrimitive() < size, "address is not within this object memory");
