...
public final class Lisp2Collector extends GarbageCollector {
     ...
     */
    private final static boolean ASSUME_SIMPLE_SLICE_TABLE = false;
    ...
    Lisp2Collector(Address ramStart, Address ramEnd) {
        ...
        Assert.always(ramSize > 0, "Lisp2Collector.java", 307);
...
////        copyTimings = new Timings();
    ...
    private static int calculateBitmapSize(int memorySize) {
        Assert.always(GC.roundDownToWord(memorySize) == memorySize, "Lisp2Collector.java", 368);
    ...
    private static int calculateMaxHeapSize(int memorySize) {
        ...
        Assert.always(memorySize >= heapSize + calculateBitmapSize(heapSize), "Lisp2Collector.java", 389);
        return heapSize;
    }
    ...
    void initialize(Address permanentMemoryStart, Address memoryStart, Address memoryEnd) {
        ...
        Assert.always(memoryEnd.roundUpToWord().eq(memoryEnd), "Lisp2Collector.java", 398);
        Assert.always(memoryStart.roundUpToWord().eq(memoryStart), "Lisp2Collector.java", 399);
        Assert.always(permanentMemoryStart.roundUpToWord().eq(permanentMemoryStart), "Lisp2Collector.java", 400);
    ...
    private void initializeHeap() {
        ...
        Assert.always(memorySize > 0, "Lisp2Collector.java", 433); // "insufficient memory for collector"
        ...
        Assert.always(heapSize >= alignment, "Lisp2Collector.java", 446); // "insufficient memory for collector"

        // Align the heap size
        heapSize = GC.roundUp(heapSize, alignment);
        heapEnd = heapStart.add(heapSize);
        Assert.always(heapSize > 0, "Lisp2Collector.java", 451); // , "insufficient memory for collector"
        ...
        if (ASSUME_SIMPLE_SLICE_TABLE) {
            Assert.always(sliceCount == 1, "Lisp2Collector.java", 509); // "ASSUME_SIMPLE_SLICE_TABLE failed"
        }

        Assert.always(sliceTable.add(sliceTableSize).loeq(alignedMemoryEnd), "Lisp2Collector.java", 512); // "slice table overflows memory boundary"
        Assert.always(sliceOffsetBits < 32, "Lisp2Collector.java", 513); // "slice size overflows 32 bit address space"
        Assert.always(bitmap.add(bitmapSize).loeq(sliceTable), "Lisp2Collector.java", 514); // "bitmap collides with slice table"
        Assert.always(heapEnd.loeq(bitmap.sub(minimumMarkingStackSize)), "Lisp2Collector.java", 515); // "heap collides with marking stack"
        Assert.always((heapSize % alignment) == 0, "Lisp2Collector.java", 516); // "heap size is non-aligned"
        Assert.always((bitmapSize % HDR.BYTES_PER_WORD) == 0, "Lisp2Collector.java", 517); // "bitmap size is non-aligned"
        Assert.always((sliceTableSize % HDR.BYTES_PER_WORD) == 0, "Lisp2Collector.java", 518); // "slice table size is non-aligned"
        Assert.always(Lisp2Bitmap.getAddressOfBitmapWordFor(permanentMemoryStart).eq(Lisp2Bitmap.getStart()), "Lisp2Collector.java", 519); // "incorrect bitmap base"
        Assert.always(Lisp2Bitmap.getAddressOfBitmapWordFor(heapEnd).lo(Lisp2Bitmap.getEnd()), "Lisp2Collector.java", 520); // "incorrect bitmap base"
    }
...
//VM.println();
    ...
    private void traverseOopsInStackChunk(Address chunk, int visitor, boolean header) throws NotInlinedPragma {
         ...
         */
        if (header) {
            if (false) Assert.that(NativeUnsafe.getAddress(chunk, SC.next).isZero());
...
////                }
                ...
                if (false) Assert.that(!copyingObjectGraph, "copied object cannot have 0 delta");
                if (GC.GC_TRACING_SUPPORTED && tracing()) {
                    VM.print("Lisp2Collector::computeAddresses - object = ");
                    VM.printAddress(object);
                    VM.print(" class = ");
...
////    }
    ...
    private Address decodeClassOrAssociationPointer(UWord encodedClassOrAssociation) {
        ...
        switch (tag) {
            ...
            default:
                if (Assert.SHOULD_NOT_REACH_HERE_ALWAYS_ENABLED) Assert.shouldNotReachHere("Lisp2Collector.java", 2266);
                return Address.zero();
        }
    }
...
////    }
    ...
    private Address compactObjects(Address firstDeadBlock, Address firstMovingBlock) throws NotInlinedPragma {
        ...
        Address object;
        if (false) Assert.that(!firstMovingBlock.isZero());
