...
//        taskCache = new Stack();
    ...
    static void reportException() throws Throwable, InterpreterInvokedPragma {
        ...
        if (false) Assert.that(thread.getHitBreakpoint() != null);
        if (false) Assert.that(thread.frameOffsetAsPointer(thread.getHitBreakpoint().hitOrThrowFO).eq(getPreviousFP(getFP())));
    ...
    static void reportBreakpoint(Offset hitFO, Offset hitBCI) throws InterpreterInvokedPragma {
        ...
        Assert.always(debugger != null, "VM.java", 631);
        try {
    ...
    static boolean _instanceof(Object obj, Klass klass) throws AllowInlinedPragma, HostedPragma  {
        if (false) Assert.that(obj != null && klass != null);
    ...
    static int arrayOopStoreCheck(Object array, int index, Object value) throws AllowInlinedPragma, HostedPragma {
        ...
        if (false) Assert.that(arrayKlass.isArray());
        if (false) Assert.that(value != null);
    ...
    static void pruneIsolateList() {
        Assert.always(isolates != null, "VM.java", 939);
...
//VM.println(" entry.isolate = null");
        ...
        Assert.always(last != null, "VM.java", 969);
...
//    private static Address timeAddr;
    ...
    private static ExecutionPoint[] reifyStack0(VMThread thread, Address fpBase, int count) {
         ...
         */
        Assert.always(stack != null, "VM.java", 1075);
    ...
    static void throwException(Throwable exception) {
        ...
        if (exceptionsEnabled) {
            ...
            if (Assert.SHOULD_NOT_REACH_HERE_ALWAYS_ENABLED) Assert.shouldNotReachHere("do_throw called recursively", "VM.java", 1182);
        }
...
//hbp.dumpState();
            ...
            if (false) Assert.that(exception == hbp.getException());
            if (false) Assert.that(hbp.getCatchMethod() != null);
        ...
        while(true) {
                ...
                if (bci.hi(start_bci) && bci.loeq(end_bci)) {
                    ...
                    if (exceptionKlass == handlerKlass || handlerKlass == Klass.THROWABLE || handlerKlass.isAssignableFrom(exceptionKlass)) {
                            ...
                            VM.getCurrentIsolate().getDebugger() != null)
                        {
                            ...
                            if (false) Assert.that(ServiceOperation.pendingException == exception);
                            ServiceOperation.pendingException = null;
                            if (false) Assert.that(throwMP == getMP(NativeUnsafe.getAddress(throwingStack, SC.lastFP)));
            ...
            fp         = getPreviousFP(fp);
            if (false) Assert.that(!fp.isZero());
            mp         = getMP(fp);
    ...
    public static int getMethodBodyLength(Object methodBody) {
        if (false) Assert.that(isValidMethodBody(methodBody));
...
//        Assert.always(stream >= STREAM_STDOUT && stream <= STREAM_STDERR); // "invalid stream specifier"
          ...
          Assert.always(stream >= STREAM_STDOUT && stream <= STREAM_SYMBOLS, "VM.java", 1743); // "invalid stream specifier"
...
//    }
    ...
    public static void printObject(Object obj) {
        if (false) Assert.that(!GC.getKlass(obj).isSquawkPrimitive());
...
//    native static long getInterruptStatus(int interrupt, int id);
    ...
    static Offset getOffsetInRom(Address object) {
        if (false) Assert.that(inRom(object));
...
////    }
    ...
    public static long makeLong(int high, int low) throws AllowInlinedPragma {
        ...
        if (false) Assert.that(getLo(value) == low);
        if (false) Assert.that(getHi(value) == high);
        return value;
    }
...
////      return taskCache;
    ...
    private static void arraycopy0(Object src, int src_position, Object dst, int dst_position, 
        ...
        if (false) Assert.that(dst != null && GC.getKlass(dst).isArray());
        if (false) Assert.that(src_position >= 0 && dst_position >= 0 && totalLength >= 0);
        if (false) Assert.that((src_position + totalLength) <=  GC.getArrayLength(src));
        if (false) Assert.that((dst_position + totalLength) <=  GC.getArrayLength(dst));
    ...
    public static void arraycopyPrimitive0(Object src, int src_position, Object dst, int dst_position, 
                                           int totalLength, int dataSize) {
        if (false) Assert.that(GC.getKlass(src) == GC.getKlass(dst));
