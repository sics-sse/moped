...
public class CS {
    ...
    public static void check(Object cs) throws AllowInlinedPragma {
        if (DEBUG) {
            Assert.always(cs != null, "CS.java", 63);
            if (GC.isSafeToSwitchThreads()) {
                Assert.always(GC.getKlass(cs).getSystemID() == CID.GLOBAL_ARRAY, "CS.java", 65);
                if  (NativeUnsafe.getObject(cs, CS.klass) != null) {
                    Assert.always(GC.getKlass(NativeUnsafe.getObject(cs, CS.klass) ).getSystemID() == CID.KLASS, "CS.java", 67);
                }
                if  (NativeUnsafe.getObject(cs, CS.next) != null) {
                    Assert.always(GC.getKlass(NativeUnsafe.getObject(cs, CS.next) ).getSystemID() == CID.GLOBAL_ARRAY, "CS.java", 70);
                }
            }
        }
    }
