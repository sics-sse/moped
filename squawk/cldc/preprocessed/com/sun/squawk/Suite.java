...
public final class Suite {
    ...
    public Klass getKlass(int suiteID) {
        if (false) Assert.that(suiteID < classes.length);
...
////          return null;
    ...
    private void installClass0(Klass klass, int suiteID) {
        ...
        if (suiteID < classes.length) {
            if (false) Assert.that(classes[suiteID] == null, klass + " already installed");
        } else {
            Klass[] old = classes;
    ...
    private void installMetadata0(KlassMetadata metadata, Klass klass, boolean replace) {
        ...
        if (metaindex >= 0) {
            if (replace) {
                ...
                if (false) Assert.that(false, "metadata for " + klass + "already installed");
            }
        } else {
            ...
            // still sorted?
            if (newIndex != 0) {
                if (false) Assert.that((klass.getSuiteID() > metadatas[newIndex - 1].getDefinedClass().getSuiteID()));
            }
            if (newIndex < metadatas.length - 1) {
                if (false) Assert.that((klass.getSuiteID() < metadatas[newIndex + 1].getDefinedClass().getSuiteID()));
            }
        }
        if (false) Assert.that(getMetadata0(klass) == metadata, "replacing: " + replace + ", metadata: " + metadata + ", getMetadata0(klass): " + getMetadata0(klass));
        checkSuite();
    }
...
//            }
    ...
    public ObjectMemory save(DataOutputStream dos, String uri, boolean bigEndian) throws HostedPragma, java.io.IOException, OutOfMemoryError {
        ...
        if (!isBootstrap()) {
            ...
            Assert.always(parentMemory != null, "Suite.java", 1006); // "parent not found: " + parent
        }
        checkSuite();
...
//                        if (false) Assert.that(count <= klass.getSuiteID());
...
//                }
    ...
    void checkSuite() {
        for (int i = 0; i < classes.length; i++) {
            Klass klass = classes[i];
            if (klass != null) {
                Assert.always(klass.getSuiteID() == i, "Suite.java", 1310);
                ...
                if (km != null) {
                    ...
                    Assert.always(km.getDefinedClass() == klass, "Suite.java", 1319);
                }
            }
        }
        ...
        if (metadatas != null) {
            ...
            for (int i = 0; i < metadatas.length; i++) {
                ...
                if (false) Assert.that(km!= null);
                Klass klass = km.getDefinedClass();
                //if (false) Assert.that(contains(klass));
                if (false) Assert.that(klass.getSuiteID() > orderCheck, " metadatas not sorted by klass suite ID");
