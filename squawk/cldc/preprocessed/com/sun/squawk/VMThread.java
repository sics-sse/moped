...
public final class VMThread implements GlobalStaticFields {
   ...
   /**
    ...
    public VMThread(Thread apiThread, String name) {
        Assert.always(apiThread != null, "VMThread.java", 264);
...
////      this.isolate  = VM.getCurrentIsolate();
    ...
    private static void addToTimerQueue(VMThread thread, long millis) {
        if (false) Assert.that(thread != null);
...
////          startFinalizers();
    ...
    private static void addToRunnableThreadsQueue(VMThread thread) {
        if (false) Assert.that(!thread.isServiceThread());
...
////      startFinalizers();
    ...
    public final void join() throws InterruptedException {
        if (this != currentThread && isAlive()) {
            ...
            if (false) Assert.that(currentThread.nextThread == null);
...
// }
            if (false) Assert.that(currentThread.waitingToJoin == null);
...
/*
    public void visitIntHashtable(int key, Object value, Object context) {
        ...
        if (t.isolate == isolate) {
            ...
            if (false) Assert.that(t == t2);
            if (false) Assert.that(t.nextThread == null);
...
////  private final static int   MAXDEPTH = Integer.MAX_VALUE;
    ...
    private void checkInvarients() throws AllowInlinedPragma {
        if (false) Assert.that(monitorDepth >= 0);
        if (false) Assert.that((monitor != null) ? (monitorDepth > 0) : true);
        if (false) Assert.that((this == serviceThread) ? (monitor == null) : true); // service thread should never be waiting for a monitor...
        if (false) Assert.that((this == serviceThread) ? (inqueue == Q_NONE) : true); // service thread should never be waiting for a anything
        if (false) Assert.that((inqueue == Q_NONE) ? (nextThread == null) : true);
        if (false) Assert.that((inqueue == Q_CONDVAR) ==  (monitor != null)); // if on condvar, then montir must be set. Otherwise monitor must be clear
        if (false) Assert.that((stack == null) ? (state == DEAD) : true);
...
//        }
    ...
    public final int getDebuggerSuspendCount() {
        if (false) Assert.that(debuggerSuspendCount >= 0);
    ...
    public final int suspendForDebugger() {
        if (false) Assert.that(debuggerSuspendCount >= 0);
        if (false) Assert.that(this != currentThread);
        return ++debuggerSuspendCount;
    }
...
//VM.println("get eventEP: " + eventEP + " debuggerSuspendCount = " + debuggerSuspendCount);
    ...
    public final void setStep(SingleStep step) {
        if (false) Assert.that(this != currentThread);
        if (false) Assert.that(this.step == null, "overlapping single step request detected too late");
        this.step = step;
    }
    ...
    void reportBreakpoint(Offset hitFO, Offset hitBCI, Debugger debugger) {
        if (false) Assert.that(hitBreakpoint == null);
...
//VM.println(VM.branchCount());
        ...
        if (false) Assert.that(exception != null);
...
//VM.println(VM.branchCount());
    ...
    void reportStepEvent(Offset fo, Offset bci) {
        ...
        if (false) Assert.that(!fo.isZero());
        Object mp = VM.getMP(frameOffsetAsPointer(fo));
        if (false) Assert.that(mp != null);
...
////    }
    ...
    private void baptiseThread() {
        if (false) Assert.that(currentThread != null);
        Assert.always(state == NEW, "VMThread.java", 1373);
...
//        }
        ...
        Assert.always(state == ALIVE, "VMThread.java", 1427);
...
//}
*/
        if (false) Assert.that(fp.hi(Address.fromObject(stack)) && fp.loeq(stackEnd));
    ...
    final Address frameOffsetAsPointer(Offset fp) throws ForceInlinedPragma {
        ...
        if (false) Assert.that(fp.ge(Offset.zero()) && fp.lt(Offset.fromPrimitive(size)));
    ...
    static boolean extendStack(int overflow) {
         ...
         */
        Assert.always(VM.isCurrentIsolateInitialized(), "VMThread.java", 1607); // "cannot extend stack until com.sun.squawk.Class is initialized"
        Assert.always(currentThread == VMThread.serviceThread, "VMThread.java", 1608);
        ...
        if (newStack == null) {
            ...
            if (false) Assert.that(GC.getKlass(NativeUnsafe.getAddress(NativeUnsafe.getAddress(newStack, SC.lastFP), FP.method)) == Klass.BYTECODE_ARRAY);
            return true;
        }
    }
    ...
    final void callRun() throws NotInlinedPragma {
        ...
        try {
            ...
            try {
                ...
                Assert.always(thread == this, "VMThread.java", 1657);
...
*/
    ...
    private static void rescheduleNext() {
        if (false) Assert.that(GC.isSafeToSwitchThreads());
...
//VM.println("Add any threads waiting for a certain time that are now due.");
            while ((thread = timerQueue.next()) != null) {
                if (false) Assert.that(thread.isAlive());
...
//VM.println("Wait for an event or until timeout..");
            ...
            if (delta > 0) {
                if (delta == Long.MAX_VALUE && events.size() == 0 && osevents.size() == 0) {
                    ...
                    if (Assert.SHOULD_NOT_REACH_HERE_ALWAYS_ENABLED) Assert.shouldNotReachHere("Dead-locked system: no schedulable threads", "VMThread.java", 1767);
                }
...
//        }
    ...
    private void abandonThread() throws NotInlinedPragma {
        if (false) Assert.that(state == DEAD); // should only be called by killThread()
...
////      startFinalizers();
    ...
    static void waitForEvent(int event) {
        waitForEvent0(event);
        reschedule();
        if (false) Assert.that(!currentThread.inQueue(VMThread.Q_EVENT) || currentThread.nextThread == null);
    }
...
//        VM.println();
//        Assert.always(object == monitor.object, "VMThread.java", 2183);
//    }
    ...
    private static boolean releaseMonitor(Monitor monitor) {
        ...
        */
        ...
        if (waiter != null /*&& waiter.isAlive()*/) {       // Is this right?
            if (false) Assert.that(waiter.isAlive());
...
//traceMonitor("fixupPendingMonitors: ", monitor, object);
                ...
                if (false) Assert.that(monitor.depth == 0);
                monitor.depth = 1;
                monitor.owner = currentThread;
            } else {
//traceMonitor("fixupPendingMonitors re-locked: ", monitor, object);

                if (false) Assert.that(monitor.owner == currentThread);
                if (false) Assert.that(monitor.depth > 0 && monitor.depth < MAXDEPTH); // startup code verifies that MONITOR_CACHE_SIZE < MAXDEPTH
...
//traceMonitor("retryMonitor: Now has the lock: ", monitor, object);
        ...
        if (false) Assert.that(monitor.owner == null);
        if (false) Assert.that(monitor.depth == 0);
       /*
        ...
        */
        ...
        if (false) Assert.that(currentThread.monitorDepth > 0);
...
//traceMonitor("monitorEnter: Got lock after waiting: ", monitor, object);        
             ...
             */
            currentThread.checkInvarients();
            if (false) Assert.that(currentThread.isolate.isExited() || monitor.owner == currentThread);
        }
    }
        ...
        if (--monitor.depth == 0) {
            if (releaseMonitor(monitor)) {
                ...
                */
                 ...
                 */
                if (false) Assert.that(monitor.owner == null && monitor.monitorQueue == null);
...
//traceMonitor("monitorExit: GC.removeMonitor: ", monitor, object);
         ...
         */
        if (false) Assert.that(monitor.owner != null || monitor.condvarQueue != null || monitor.monitorQueue != null || monitor.depth == 0);
        currentThread.checkInvarients();
    }
...
//traceMonitor("monitorWait: released monitor: ", monitor, object);
        ...
        if (false) Assert.that(monitor.condvarQueue != null);
        if (false) Assert.that(theCurrentThread.monitor == monitor);
        reschedule();
...
//traceMonitor("monitorWait: woke up and re-locked: ", monitor, object);
        ...
        // Safety...
        theCurrentThread.checkInvarients();
        if (false) Assert.that(monitor.owner == theCurrentThread);
    }
    ...
    public static void monitorNotify(Object object, boolean notifyAll) {
        ...
        if (VM.hasVirtualMonitorObject(object)) {
            if (false) Assert.that(!GC.hasRealMonitor(object));
...
//        }
        ...
        if (false) Assert.that(inqueue == expectedQueue);
    }
...
//VM.print(" name=");
//VM.println(name);
        if (false) Assert.that(isServiceThread() ? name == Q_NONE : true); // service thread waits for no-one?

        if (false) Assert.that(monitor == null); // monitor should only be set after setInQueue(Q_CONDVAR)
        if (false) Assert.that(inqueue == Q_NONE);
...
//VM.print(" name=");
//VM.println(name);
        if (false) Assert.that(inqueue == name);
...
final class TimerQueue {
    ...
    VMThread next() {
        ...
        thread.nextTimerThread = null;
        if (false) Assert.that(thread.time != 0);
        thread.time = 0;
        return thread;
    }
