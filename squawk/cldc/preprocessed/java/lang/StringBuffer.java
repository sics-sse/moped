...
public final class StringBuffer {
    ...
    private final static int UNKNOWN_ENCODING = 2;
    ...
    public StringBuffer(int length) {
        if (false) Assert.that(encoding == IS_EIGHT_BIT); // the null char is 8-bit
    ...
    public synchronized StringBuffer append(char str[]) {
        ...
        count = newcount;
        if (encoding == IS_EIGHT_BIT && !String.isEightBitEnc(str, 0, len)) {
            encoding = IS_NOT_EIGHT_BIT;
        }
        return this;
    }
    ...
    public synchronized StringBuffer delete(int start, int end) {
        ...
        if (encoding != IS_EIGHT_BIT) {
            // if encoding was 16 bit, we may have just deleted the only 16-bit character, so
            // it may be 8 bit now. Check later.
    ...
    final boolean isEightBit() {
        ...
        // remove uncertainty
        synchronized (this) {
            if (false) Assert.that((encoding & UNKNOWN_ENCODING) != 0);
