...
public class SSLStreamConnection implements StreamConnection {
    ...
    private byte version;   // SSL version to use
    private long hstime;    // Total time taken for handshake
    private String hstype;  // New handshake or session reuse + session ID
    ...
    public SSLStreamConnection(String host, int port,
        ...
        try {
            ...
            hndshk.doHandShake(Record.CLIENT, (byte) 0x31);
            serverCert = hndshk.sCert;
            cipherSuite = hndshk.negSuiteName;
            version = hndshk.getNegotiatedVersion();
            hstype = hndshk.getType();
            hstime = hndshk.getTimeMillis();
	        verificationChain = hndshk.verificationChain;
            // System.out.println(new SSLSecurityInfo(this));
        } catch (IOException e) {
            cleanupIfNeeded();
            throw e;
        }
    ...
    public SSLStreamConnection(String host,
            InputStream in, OutputStream out,
            CertStore cs)
            throws IOException {
        ...
        try {
            Handshake hndshk = new Handshake(host, port, rec, cs);
            hndshk.doHandShake(Record.SERVER, (byte) 0x31);
            serverCert = hndshk.sCert;
            cipherSuite = hndshk.negSuiteName;
            version = hndshk.getNegotiatedVersion();
            hstype = hndshk.getType();
...
class SSLSecurityInfo implements SecurityInfo {
    ...
    public String getProtocolVersion() {
        int version=parent.getVersion();        
        return ((version&0xff)>>4)+"."+(version&0x0f);
    ...
    public long getHandshakeTimeMillis() {
        return parent.getHandshakeTimeMillis();
    }

    public String getHandshakeType() {
        return parent.getHandshakeType();
    }

    public String toString() {
        String res;

	    X509Certificate certificate = (X509Certificate) getServerCertificate();
        ...
        res = "Security:      On" +
          ...
          "\nHandshakeTime: " + getHandshakeTimeMillis() + "ms" +
		  "\nCertificate:   " + getServerCertificate().getSubject() +
		  "\nFingerprint:   " + Utils.hexEncode(certificate.getFingerprint()) +
		  "\nCert path:     ";
	    String[] validationPath = parent.verificationChain;
	    for(int i = 0; i < validationPath.length; ++i) {
		    res += validationPath[i];
		    res += "\n               ";
	    }
        return res;
    }
