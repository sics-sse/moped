...
class Record {
    /*
     * TODO: We should try to avoid multiple buffer copies by defining a 
    ...
    /** The SSL version in one byte (0x31=3.1). */
    private byte ver;
    ...
    Record(InputStream ins, OutputStream outs) {
        in = ins;
        out = outs;
        ver = (byte) 0x31;  // TODO: This is hardcoded for now
    }
    
    /** sets the negotiated version in the handsahke
     * to the record layer
     */
    void setNegVersion(byte version ) {
	ver = version;
    }
    ...
    /**
     * Init method for version 3.1
     * <P />     * @param crand 32-byte random value chosen by the client
     * @param srand 32-byte random value chosen by the server
     * @param suite negotiated cipher suite
     * @param master master secret resulting from the key exchange
     * @exception Exception if the negotiated cipher suite involves an 
     * unsupported hash or cipher algorithm
     */


    void init31(byte[] crand, byte[] srand, 
              byte suite, byte[] master) throws Exception {

	//TODO: COMP_30 means what differences are there in comparison to the contemporate init30
	//TODO: COMP_30: 30 sets padLen as well. Only used in getMAC (not in getMAC31), thus
			// as a first hack it's ok to initialize it for 31 as well.
...
/*        int keyMat = 5;
	...
	int ivSize = 0; // stream ciphers do not use IVs
        ...
        int i = (dLen + keyMat + ivSize) *2;
	byte[] keyBlock;
	byte temp[] = new byte[crand.length + srand.length];
	System.arraycopy(srand, 0, temp, 0, srand.length);
	System.arraycopy(crand, 0, temp, srand.length, crand.length);
	keyBlock = Handshake.PRF(master,"key expansion", temp, i );

	//splitting the keyBlock into keys
	
	cMACKey = new byte[dLen];
	sMACKey = new byte[dLen];
	cKey = new byte [keyMat];
	sKey = new byte [keyMat];
        cIV = new byte[ivSize]; // they are zero
        sIV = new byte[ivSize];


	System.arraycopy(keyBlock, 0, cMACKey, 0, dLen);
	System.arraycopy(keyBlock, dLen, sMACKey, 0, dLen);
	System.arraycopy(keyBlock, dLen *2, cKey, 0, keyMat);
	System.arraycopy(keyBlock, dLen *2 + keyMat, sKey, 0, keyMat);

	//additional  processing for exportable code 
	if (suite == Handshake.ARCFOUR_40_MD5 ) {
	    cKey = Handshake.PRF(cKey,"client write key", temp,keyMat);
	    sKey = Handshake.PRF(sKey,"server write key", temp,keyMat);
	}
*/
	//TODO: COMP_30: rest is equal
        /*
         * Now initialize the ciphers and keys. For now this is always
         * ARCFOUR and we comment out support for other ciphers.
         */ 
  /*      cBulkKey = (SecretKey) KeyBuilder.buildKey(KeyBuilder.TYPE_ARCFOUR,
                                           (short) (cKey.length << 3), false);
        cBulkKey.setKey(cKey, (short) 0);
        
	cc = Cipher.getInstance(Cipher.ALG_ARCFOUR, false);
        cc.init(cBulkKey, Cipher.MODE_ENCRYPT); 

        sBulkKey = (SecretKey) KeyBuilder.buildKey(KeyBuilder.TYPE_ARCFOUR,
                                           (short) (sKey.length << 3), false);
        sBulkKey.setKey(sKey, (short) 0);
        sc = Cipher.getInstance(Cipher.ALG_ARCFOUR, false);
        sc.init(sBulkKey, Cipher.MODE_DECRYPT); 
...
*/            
    ...
    private boolean rdRec(boolean block) throws IOException {
        ...
        if (rActive == 1) {
            try {
                //System.out.println("rCypher:\n" + PrettyPrint.prettyPrint(inputData, 0, dataBytesRead));
        ...
        //System.out.println("rPlain:\n" + PrettyPrint.prettyPrint(inputData, 0, plainTextLength));
...
//        // Don't print header info to match the contents of rPlain, rCypher
//        System.out.println("wPlain:\n" + PrettyPrint.prettyPrint(rec, 5, len));
//        if (wActive == 1) {        
//			System.out.println("wCypher:\n" + 
//				PrettyPrint.prettyPrint(tmp, 5, (tmp.length - 5)));
//        }
        
        //Flush is required for Sun SPOTs. 
        //TODO: Check whether this is a bug in the SPOTs stream implementation.
        out.flush(); 
...
class CipherSuiteData {
    ...
    private SecretKeySpec clientBulkKey = null;
    /** Servers bulk encryption secret key. */
    private SecretKeySpec serverBulkKey = null;
    ...
    /**
     * Constructs CipherSuiteData object for ssl 3.0
     * 
     * @param suite negotiated cipher suite
     *
     * @exception Exception if the negotiated cipher suite involves an 
     ...
     * @param suite negotiated cipher suite
     * 
     * @param ssl/tls version (0x30 for ssl 3.0 or 0x31 for tls)
     *
     * @exception Exception if the negotiated cipher suite involves an 
     *                      unsupported hash or cipher algorithm
     */       
    CipherSuiteData(short suite, byte version) throws Exception {
        suiteType = suite;
        ...
        /*
         * TODO: We should separate out the key exchange mechanism from
         * the bulk encryption/authentication mechanism so if we add a new
         * suite that also uses RC4_128_SHA, we shouldn't have to add a new
         * "case" here.
         */
        switch (suite) {
        case Handshake.ARCFOUR_128_MD5:
        ...
        case Handshake.ARCFOUR_128_SHA:
        case Handshake.ECDH_ECDSA_RC4_128_SHA:
        case Handshake.ECDHE_ECDSA_RC4_128_SHA:
        case Handshake.ECDH_RSA_RC4_128_SHA:
        case Handshake.ECDHE_RSA_RC4_128_SHA:
            keyMaterial = 16;
            padLength = 40;
        ...
        //  The TLS implementation doesn't use padLength. (TODO: find out why this was changed) 
        if (version>=0x31)
            padLength=0;  
        ...
        //TODO: The TLS version creates the key block array
        // when it initializes it. Thus the following
        // code is usesless and creates only more garbage for TLS.
    ...
    private void generateKeys31(byte[] crand, byte[] srand, 
              byte[] master) {
...
*/
	int ivSize = 0; // stream ciphers do not use IVs           
        ...
        int i = (digestLength + keyMaterial + ivSize) *2;
//	byte[] keyBlock;
	byte temp[] = new byte[crand.length + srand.length];
	System.arraycopy(srand, 0, temp, 0, srand.length);
	System.arraycopy(crand, 0, temp, srand.length, crand.length);
...
*/
/*
	System.arraycopy(keyBlock, 0, cliMACKey, 0, dLen);
	System.arraycopy(keyBlock, dLen, sMACKey, 0, dLen);
	System.arraycopy(keyBlock, dLen *2, cKey, 0, keyMat);
	System.arraycopy(keyBlock, dLen *2 + keyMat, sKey, 0, keyMat);
	...
	//additional  processing for exportable code 
	if (suiteType == Handshake.ARCFOUR_40_MD5 ) {
	    cKey = Handshake.PRF(cKey,"client write key", temp,keyMat);
	    sKey = Handshake.PRF(sKey,"server write key", temp,keyMat);
	}*/
...
*/            
	...
	if (version==0x30) {
	   ...
	   generateKeys31(clientRand, serverRand, masterSecret);
	   //TODO: With some luck this works. However, chopKeysBlock
	   //also contains IV generation. This WON'T work, but is currently
	   //only used for the export cipher. (But we will need it for AES)
	   chopKeysBlock(clientRand, serverRand, masterSecret);
	    //TODO: refactor
     ...
     */
    SecretKeySpec getClientBulkKey() {
     ...
     */
    SecretKeySpec getServerBulkKey() {
    ...
    private void chopKeysBlock(byte[] clientRand, byte[] serverRand,
        ...
        if (suiteType == Handshake.ARCFOUR_128_MD5 ||
            ...
            suiteType == Handshake.ECDH_ECDSA_RC4_128_SHA ||
            suiteType == Handshake.ECDHE_ECDSA_RC4_128_SHA ||
            suiteType == Handshake.ECDH_RSA_RC4_128_SHA ||
            suiteType == Handshake.ECDHE_RSA_RC4_128_SHA) {
        /*
        ...
        /* 
         * TODO: The only other option is ARCFOUR_40_MD5
         * 
         ...
         */
            if (version == 0x31)  
                throw new RuntimeException("ARCFOUR_40_MD5 not " +
                  "supported in TLS.");
         ...
         * ARCFOUR and we comment out support for other ciphers.
         */ 
        clientBulkKey = new SecretKeySpec(clientKey, 0, clientKey.length, "ARC4");
        serverBulkKey = new SecretKeySpec(serverKey, 0, serverKey.length, "ARC4");
    }
}
...
class MAC {
	...
	if (version == 0x31) 
	    return getMac31(type, buf, offset, length);
    ...
    /**
     *  Calculates MAC for TLS aka SSlv3.1 messages
     * <P />
     * @param who one of CLIENT or SERVER to indicate whether 
     * client-side or server-side MAC secrets and sequence numbers
     * ought to be used in the MAC computation
     * @param type SSL record type of the SSLCompressed structure
     * @param buf byte array containing the SSLCompressed fragment
     * @param off starting offset of the fragment in buf
     * @param len length of the fragment
     * @return a byte array containing the MAC
     */

 
    private byte [] getMac31(byte type, byte[] buf, int off, 
                          int len) {

	//byte alg = digest.getAlgorithm(); //this value can be set as a constant when d is assigned 

        byte[] seqArr=Utils.longToBytes(sequenceNumber);
	//byte[] macWriteKey;
/*	if (who == CLIENT) {
	  //tmp=seqNum + TLSCompressed.type + TLSCompressed.version + TLSCompressed.length + TLSCompressed.fragment(len)
	    seqArr = Utils.longToBytes(cSeq);  
	    //macWriteKey = cMACKey;

        } else {

	    seqArr = Utils.longToBytes(sSeq);  
	    //macWriteKey = sMACKey;
	...
	byte[] res = new byte[5];
	res[0] = type;
        res[1] = (byte)(version >>> 4);
	res[2] = (byte)(version & 0x0f);
        res[3] = (byte) (len >>> 8);
        res[4] = (byte) (len & 0xff);

        byte[] temp = new byte[ seqArr.length + res.length + len];
	System.arraycopy(seqArr, 0, temp, 0, seqArr.length);
	System.arraycopy(res, 0, temp, seqArr.length, res.length);
	System.arraycopy(buf, off, temp, seqArr.length + res.length, len);
        return Handshake.hmac(digest.getAlgorithm(), macSecret, temp); //TODO: save to use messagedigest name?
