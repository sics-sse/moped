...
import com.sun.spot.security.implementation.ModuloExponentiation;
import com.sun.spot.security.implementation.RSAKey;
...
public final class RSA extends CipherSpi {
    protected final static int  MODE_UNINITIALIZED=0; 
    protected final static String RSA_ALGORITHM_NAME = "RSA";
    ...
    /**
     * An extremley slow method for performing modular exponentiation
     * using the FFA class.
     *
     ...
     * @param reslen    the length of the result is returned here.
     *
     */
    
    private static int modExpFFA(byte [] data, byte[] exponent,
				      byte[] modulus, byte[] result) throws IllegalArgumentException {

	FFA ffa=new FFA(modulus.length*8);
	int[] a=ffa.from(data, 0, data.length);
	int[] e=ffa.from(exponent, 0, exponent.length);
	int[] m=ffa.from(modulus, 0, modulus.length);
	int[] r=ffa.acquireVar();
	
	ffa.modPow(r, a, e, m);	

	ffa.toByteArray(result, 0, result.length, r);
	return ffa.getByteSize();
    }
    ...
    private byte[] doIt(byte[] data) {
        ...
        // Using modExpFFA instead of ModuloExponentiation.oddModPow
        // (and removing the ModuloExponentiation class) reduces
        // library size significantly on the cost of a much lower
        // RSA performance. 
        
        //bufLen = modExpFFA(data, exp, mod, buf); 
        bufLen =  ModuloExponentiation.oddModPow(buf, data,exp,mod);
     ...
     */
    protected void engineSetChainingModeAndPadding(String mode, String padding)
    ...
    private void addToMessage(byte inBuf[], int inOff, int inLen)
        ...
        if (mode == RSA.MODE_UNINITIALIZED) {
    ...
    private int performRsa(byte inBuf[], int inOff, int inLen,
        ...
        case Cipher.ENCRYPT_MODE:
            ...
            if (ckey instanceof RSAPublicKey) {
                ...
                rnd.generateData(tmp, PAD_OFFSET, padLen);
