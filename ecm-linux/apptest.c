...
uint8* Can_Read_Package (uint32 can_socket, uint32 can_ID){
    //uint8 dataLength = 0;
	...
	setsockopt(can_socket, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(struct timeval));
    ...
    while(packageReadDone == false){
		...
		if(canID != can_ID){
			...
			switch(frameType){
				case SINGLE_FRAME:
					//dataLength = frame.data[0] & 0x0F;
					//package = (uint8 *) malloc( sizeof(uint8)*dataLength );
