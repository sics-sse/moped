...
static Spi_TransferStatus Spi_Rx_FIFO(Spi_UnitType *spiUnit) {
	...
	/* Copy data from FIFO to receiving channels */
	  ...
	  //printf("Spi_Rx_FIFO jobUnitPtr->fifoSent = %d\r\n", jobUnitPtr->fifoSent);
	  ...
	  for (i=0; i<copyCnt; i++) {
	    ...
	    //printf("Spi_Rx_FIFO data %d\r\n", i);
	    popVal = SPI0_FIFO;
	    //printf("Spi_Rx_FIFO read %d\r\n", popVal);
...
static void Spi_WriteJob_FIFO(Spi_JobType jobIndex)
{
    ...
    //printf("Spi_WriteJob_FIFO %d\r\n", currChannel);

    while (fifoLeft != 0) {
      //printf("Spi_WriteJob fifoLeft = %d\r\n", fifoLeft);
    	/* Get tx-buffer */
	Spi_pos = 2;
	...
	Spi_pos = 42;
		  ...
		  ii = (buf[jobUnitPtr->txChCnt] << 8) + (buf[jobUnitPtr->txChCnt + 1] & 0xff);
                } else {
                	ii = buf[jobUnitPtr->txChCnt];
                }
		SPI0_FIFO = ii;
            } else {
            	SPI0_FIFO = chConfig->SpiDefaultData;
		//printf("spi wrote def %d\r\n", chConfig->SpiDefaultData);
            }
